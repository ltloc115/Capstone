...
// Frog
void ble_phone_connection_status()
{
  ...
  if (deviceConnected) {
    ...
    value++;
    Serial.println("Bluetooth Phone Connected");

    phoneconnect = true; // set the boolean as true when bluetooth is connected
  ...
  if (!deviceConnected && oldDeviceConnected) {
    ...
    phoneconnect = false; // set the boolean as true when bluetooth is connected
  }
...
// Additionally the SD card

void bluetooth_startup() {
  
  delay (500);


  /************* Initializing Bluetooth classic with speakers *************/
  if (!btStart()) {
  ...
  /************* Binh start Bluetooth connection *************/

  // Create the BLE Device
  BLEDevice::init("Karthi the Man");
  ...
  // this is the call back shit that I added
  // FROG
  ...
  /***Bluetooth connection for Speakers ***/
  ...
  //
  //  sdmmc_card_t* card;
  //  esp_err_t ret2 = esp_vfs_fat_sdmmc_mount("/sdcard", &host, &slot_config, &mount_config, &card);
  //
  //  if (ret2 != ESP_OK) {
  //    if (ret == ESP_FAIL) {
...
/****** Tien's code to connect to Speakers*******/
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
  ...
  for (int i = 0; i < param->disc_res.num_prop; i++) {
    ...
    switch (p->type) {
      ...
      case ESP_BT_GAP_DEV_PROP_EIR:
        ...
        ESP_LOGI(BT_AV_TAG, "--name %s", peer_bdname);
        break;
  ...
  */
  ...
  if (eir) {
    get_name_from_eir(eir, peer_bdname, NULL);
    if (strcmp((char *)peer_bdname, "JBL GO 2") != 0) {
      return;
    }
